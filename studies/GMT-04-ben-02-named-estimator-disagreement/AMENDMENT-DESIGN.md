...
difference between the IPTW and g-formula point estimates in C0. The same scratch script also computed
that difference in two primary scenarios; those lines were not read. Every change below is justified by
a validity gate, a calibration control or cost, not by any primary-scenario disagreement.

## 1. Why the registered design cannot answer

**The stated block, compute, is overstated about sevenfold.** BLOCKED.md quotes 110 to 470 seconds per
replicate and about 1500 CPU-hours. Those are elapsed times: the harness's `secs` column is
...
- *C0 calibration gate* (upper 95% bound for Pr(range of the three RDs > 0.03) below 0.20). The range
  ...
  adds its own spread. The calibration control would fail at 1200 and very likely at 4000, and the study
  would be uninformative either way. The 0.03 range threshold and the 0.20 probability threshold cannot
...
4. **Timing pilot as a validity stop.** The registered pilot (10 replicates in each of four cases)
   runs at n = 8000 with the amended seeds' pilot stream, together with 10 replicates of C0 and of C1. If any primary scenario in it has fewer than
   10 of 10 complete triplets, C0 has 4 or more of 10 replicates with D = 1, or C1 has fewer than 5 of
   10, the main run does not start and a further amendment is required. These are validity and calibration quantities, not the
   primary endpoint.
...
- **C0: upper bound of Pr(D = 1) below 0.20.** Scaling the measured standard deviation of the IPTW
  minus g-formula difference by √(4000/8000) gives 0.0125 at n = 8000, so Pr(|IPTW − g-formula| > 0.03)
  ≈ 2Φ(−2.4) = 0.016. If ltmle's spread raises Pr(D = 1) to 0.10, the Wilson upper bound at R = 500 is
...
- **C1: lower bound of Pr(D = 1) above 0.20.** C1 doubles the omitted transition and event interactions
  ...
  asymptotic bias, which does not shrink with n; but if that difference is below 0.03, a larger n drives
  Pr(D = 1) in C1 toward zero and the gate fails. The size of the difference was not checked here. At
  R = 500 the gate needs Pr(D = 1) of about 0.24 or more, so the pilot stop in change 4 also requires at
  least 5 of 10 C1 pilot replicates with D = 1.
...
| Bootstrap: 4 anchors × 25 × 400 IPTW point refits | 40,000 | 0.5 | 5.6 |
