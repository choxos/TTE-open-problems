...
3. **A registered, outcome-blind calibration of the poor-separation level and the nonlinear-treatment
   ...
   true-weight P(min regime ESS < 10) is at most 0.02 (Monte Carlo SE 0.0044). Any change to the
   mechanism after calibration, including the stage 2 scenarios, uses the frozen values.
   - s_poor is the largest value in {0.85, 0.75} meeting the criterion in scenarios 2, 14 and 20, the
     ...
     and n = 8000 without the intercept shift they were already 0.010, 0.020 and 0.005. If neither value passes, the
   ...
   - The I_G term becomes c_G × I_G{0.45(L_t² − 1) + 0.35 W1 L_t}, with c_G the largest value in
     {1, 0.75, 0.5} meeting the criterion in scenarios 7 and 8 at the chosen s_poor; scenarios 1, 13 and
     19 do not contain the term and must meet the criterion as they are. Scenario 7 measured 0.047 at
     c_G = 1 with the amended intercept at n = 8000, so c_G = 1 will fail and the outcome is 0.75, 0.5
     or removal of the shape. The functional form is unchanged, so candidates G2 and G4 remain correctly
...
- **Combined-selector convergence, Wilson lower bound ≥ 0.93 in every γ_Z = 0 scenario (positive
  ...
  primary scenario by construction, or the study stops before drawing a replicate. Allowing
  estimated-weight failure to be 1.5 times the true-weight rate (the feasibility run gave 1.3 times),
  convergence is at least 0.97, and at 2000 replicates its Wilson lower bound is 0.962, a margin of
  0.032. Measured values indicating the calibration can pass outside the nonlinear-treatment cells: 0.020 (scenario 2, s = 0.85), 0.013 (14, s = 0.85) and 0.007 (13,
...
| Calibration, 8 γ_Z = 0 scenarios × 1000 mechanism-only datasets × up to 3 grid values | | about 2 |
...
- **Poor separation is milder.** s = 0.85 or 0.75 instead of 1.25, and treatment prevalence falls from
  53% to 61% to about 40% to 47%. The study no longer says anything about selection when the never-treat
...
The checks behind section 1 used an estimated 30 to 40 CPU-minutes (not metered), more than the 15 allowed, partly because two
