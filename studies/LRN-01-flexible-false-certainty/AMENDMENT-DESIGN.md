...
- **Both primary branches are reachable.** The decision family has 7 cells × 2 comparators = 14
  ...
  most 0.7, the bound half-width at R = 600 is 2.69 × 0.7/√600 = 0.077. "Supported" needs a true excess
  of at least about 0.18 in every cell; F_flex can be as large as P(green), which is at least 0.5 by
  construction, so excesses of that size are possible. "Not supported" needs, at a true excess of zero, a paired standard deviation of at most
...
- **Support-map branches.** Sensitivity and specificity use Wilson bounds with Bonferroni over 7
  ...
  observed rate of 0.96 at R = 600 has a lower bound of about 0.935, so the success thresholds of 0.90
  are reachable; failure (upper bound ≤ 0.80) is reachable by an observed rate near 0.74. Whether the
...
- **Observed treatment patterns.** With κ_A of 7 or more, the observed probability of continuing the
  previous action is mostly above 0.9 at every visit outside the soft-overlap scaling, so observed
  treatment is nearly sustained after the first visit.
