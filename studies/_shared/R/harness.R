...
## Runtime guards against silent defects. Located relative to this file so a
## study sourcing the harness from any working directory picks them up.
local({
  here_file <- tryCatch(normalizePath(sys.frame(1)$ofile), error = function(e) NULL)
  guards <- if (!is.null(here_file)) file.path(dirname(here_file), "guards.R") else NULL
  if (is.null(guards) || !file.exists(guards)) stop("harness.R: cannot locate guards.R")
  sys.source(guards, envir = globalenv())
})
...
run_design <- function(fn, scenarios, n_rep, master_seed, outdir,
  ...
  ## Smoke mode: TTE_SMOKE=k runs the first k replicates of each selected
  ## scenario in this process, writes nothing, and returns them. It draws from
  ## the registered streams, so it previews exactly what the run will compute,
  ## and it is how every study is checked before it is launched.
  smoke <- suppressWarnings(as.integer(Sys.getenv("TTE_SMOKE", "")))
  if (!is.na(smoke) && smoke > 0L) {
    out <- lapply(todo, function(s) {
      idx <- (s - 1L) * n_rep + seq_len(min(smoke, n_rep))
      res <- do.call(rbind, lapply(seq_along(idx), function(i)
        run_one(fn, scenarios[s, , drop = FALSE], i, streams[[idx[i]]])))
