...
L_t = 1 throughout add to it. C_1(κ) therefore declines only slowly toward that mass: the targets 0.08,
0.04 and 0.02 cannot be reached by any κ, and 0.12 only near κ = 6, far outside the registered grid.
...
0.0017, so even at n = 16,000 and C_min near 0.13 the weaker arm expects about 3.5 events per late
cell, each such cell is empty with probability about e⁻³·⁵ = 0.03, the chance that none of the weaker
arm's 120 endpoint-cells is empty is well under 5%, and the model returns `outcome-separation`. With the three defects patched in a scratch copy, a yearly collapse of the cells
...
exact-regime methods 1.7 s at n = 1000, 2.5 s at n = 4000 and 8.6 s at n = 16,000; the MSM with yearly
piecewise-constant baseline hazards (10 cells per endpoint, a stand-in for the spline in change 3 with
a similar parameter count) adds 4.3 s at n = 4000 and 16.8 s at n = 16,000. That is roughly 11 s per
replicate on average, about 2200 CPU-hours for the core and validation replicates alone; the sentinel
bootstrap's 25.5 million MSM refits add at least ten times more. The in-code "overnight profile" (20 replicates) could never classify a
...
- **Calibration (0.01 tolerance).** With 20,000 histories the amended mechanism reaches C_min = 0.1209,
  ...
  below the lowest target. The 20,000-history estimates have Monte Carlo error of a few thousandths at
  most, so the registered 2,000,000-history calibration cannot close the 0.009 margin.
...
- **At least 0.9R valid exact_fixed intervals per cell.** Apart from a treatment-model failure, which
  the correctly specified logistic models make rare, exact_fixed fails only on a zero arm denominator
  or overflow. The smallest expected compatible count is n × C_min × subgroup share =
  about 1000 × 0.02 × 0.25 = 5 at 60 months in the G1 subgroup at K4, so P(zero denominator) ≈ e⁻⁵ =
  0.007 and availability is about 0.99 even there. The overall cells have 20 or more.
...
- **Interval failure and adequacy are both reachable at R = 1000.** With 1000 intervals, observed
  coverage 0.88 gives a Wilson upper bound of 0.899 (below 0.90: failure), and observed coverage 0.945
  gives a lower bound of 0.929 (at least 0.925: adequate); 0.94 gives 0.9235 and is indeterminate. The
  registered example (coverage near 0.85 or 0.95) still separates. At R = 500 (the n = 16,000 arm) the
  corresponding observed values are 0.87 (upper bound 0.897) and 0.95 (lower bound 0.927).
- **Support attribution.** The K0 comparison uses common random numbers. The coverage difference for
  independent arms at 0.95 and 0.90 has standard error about 0.0117 at R = 1000; the required upper
...
The checks behind sections 1 and 3 (scratch scripts `compat_curve.R`, `compat_curve2.R`, `debug.R`,
`timing.R` and `timing2.R`, run on scratch copies of `01-dgm.R` and `02-estimators.R`) used about
8 CPU-minutes and wrote nothing under `results/`.
